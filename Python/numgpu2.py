...
__kernel void add(__global const long *a_g, __global long *res1_g, __global long *res2_g, __global long *res11_g)
{
    int gid = get_global_id(0);
    res1_g[gid] = num() + (a_g[gid]*a_g[gid]*a_g[gid]);
    res2_g[gid] = num() - (a_g[gid]*a_g[gid]*a_g[gid]);
    res11_g[gid] = -1*num() + (a_g[gid]*a_g[gid]*a_g[gid]);
}
...
__kernel void fac(__global const long *a_g, __global long *j_g, __global long *k_g)
{
    ...
    for(i=0;i<c;i++)
    {
        ...
        {
            if( abs((j*j)-(j*(temp.num1-j))+((temp.num1-j)*(temp.num1-j))) == temp.num2)
            {
                j_g[gid]=j;
                k_g[gid]=temp.num1-j;
                flag=1;
...
def mainfun(k):
    ...
    prg.add(queue, a_np.shape, None, a_g, res1_g, res2_g, res11_g)
