    ...
    def sensor_read(self, unused):
        ...
        while True:
            ...
            if self.mpu_flag == 1:
                ...
                di_temp['mpu6050']['ax'] = round(sum([x[0] for x in mpu_val])/len(mpu_val), 1)
                di_temp['mpu6050']['ay'] = round(sum([x[1] for x in mpu_val])/len(mpu_val), 1)
                di_temp['mpu6050']['az'] = round(sum([x[2] for x in mpu_val])/len(mpu_val), 1)
                di_temp['mpu6050']['gx'] = round(sum([x[3] for x in mpu_val])/len(mpu_val), 1)
                di_temp['mpu6050']['gy'] = round(sum([x[4] for x in mpu_val])/len(mpu_val), 1)
                di_temp['mpu6050']['gz'] = round(sum([x[5] for x in mpu_val])/len(mpu_val), 1)
